  ...
  <section class="activity-page" dir="rtl">
        ...
        <table class="activity-table">
          ...
          <tbody>
            <tr v-for="log in activityLogs" :key="log.id">
              ...
              <td>
                <button type="button" class="activity-details-btn" @click="activeLog = log">
                  مشاهده
                </button>
              </td>
            </tr>
    ...
    <div v-if="activeLog" class="activity-modal-backdrop" @click.self="activeLog = null">
      <section class="activity-modal" role="dialog" aria-modal="true" aria-labelledby="activity-detail-title">
        <header>
          <div>
            <small>{{ activeLog.event_label || activeLog.event }} در {{ activeLog.section || '-' }}</small>
            <h3 id="activity-detail-title">{{ activeLog.subject_label || activeLog.subject_id || 'جزئیات تغییرات' }}</h3>
          </div>
