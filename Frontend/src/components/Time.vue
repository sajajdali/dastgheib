          ...
          >
              ...
              >
                ...
                <date-picker
                  ...
                  @update:model-value="sortDayRowsByTime(day); persistDirectAppointment(row)"
                />
  ...
  <select v-model="row.done" @change="onDoneChanged(row)">
    ...
    <div v-if="appointmentView === 'timeline'" class="timeline-actions" @click.stop>
      ...
      <div class="booking-timeline-filters" aria-label="فیلتر وقت‌دهی خدمات">
        ...
        <button type="button" class="booking-filter-apply" :disabled="timelineScheduleLoading" @click="applyBookingTimelineFilters">{{ timelineScheduleLoading ? 'در حال اعمال…' : 'اعمال فیلتر' }}</button>
        <button v-if="bookingTimelineFilterActive" type="button" class="booking-filter-clear" :disabled="timelineScheduleLoading" @click="clearBookingTimelineFilters">پاک کردن</button>
        ...
        <div class="timeline-slots" :aria-label="`نوبت‌های ${day.dateLabel}`">
          <template v-for="(row, index) in day.timelineRows" :key="row._rowId">
            <article
              ...
              :style="timelineBookingCardStyle(row)"
      ...
      <section class="financial-panel" @click.stop>
        ...
        <section class="financial-invoice-card">
          ...
          <div v-if="financialServicesOpen" class="financial-invoice-table-wrap">
            <table><thead><tr><th>خدمت</th><th>تعداد</th><th>مبلغ</th><th>تخفیف</th><th>خالص</th></tr></thead><tbody>
              <tr v-for="line in financialInvoiceLines" :key="line.key"><td>{{ line.isAddon ? '↳ ' : '' }}{{ line.service }}<small>{{ [line.section,line.subsection].filter(Boolean).join(' / ') }}</small></td><td>{{ line.quantity }}</td><td>{{ formatDisplayMoney(line.grossAmount) }}</td><td>{{ formatDisplayMoney(line.discountAmount) }}</td><td>{{ formatDisplayMoney(line.netAmount) }}</td></tr>
              <tr v-if="!financialInvoiceLines.length"><td colspan="5">خدمتی برای این نوبت ثبت نشده است.</td></tr>
            </tbody></table>
            <div class="financial-payable-total"><span>مبلغ قابل پرداخت</span><strong>{{ formatDisplayMoney(moneyToNumber(activeFinancialRow?.amount)) }} تومان</strong></div>
          </div>
        </section>
        <button
        ...
        <section class="financial-accordion-section payment-open">
        <button type="button" class="financial-accordion-head"><span>پرداخت</span><b>باز</b></button>
        <div class="financial-accordion-body financial-payment-body">
...
export default {
  ...
  data() {
    return {
      ...
      financialServicesOpen: true,
      financialDebtOpen: false,
      financialDepositOpen: false,
      ...
      draftBookingRootSectionFilter: "",
      draftBookingServiceFilter: "",
      draftBookingResourceFilter: "",
  ...
  computed: {
    ...
    enabledBookingRootSectionOptions() {
      const roots = new Map();
      this.enabledBookingSectionOptions.forEach(section => {
        const rootId = String(this.rootSectionIdFor(section.id) || section.id);
        const root = (this.serviceSections || []).find(item => String(item.id) === rootId);
        roots.set(rootId, { id: rootId, label: String(root?.name || this.bookingSectionPathLabel(rootId) || section.label) });
      });
      return [...roots.values()].sort((a, b) => a.label.localeCompare(b.label, 'fa'));
    },
    bookingSubsectionFilterOptions() {
      return this.bookingSectionsForFilters(this.draftBookingRootSectionFilter, '');
    },
    bookingResourceOptions() {
      return this.bookingResourcesForFilters(this.bookingRootSectionFilter, this.bookingServiceFilter);
    },
    bookingResourceFilterOptions() {
      return this.bookingResourcesForFilters(this.draftBookingRootSectionFilter, this.draftBookingServiceFilter);
    },
    bookingTimelineFilterActive() {
      return Boolean(this.bookingRootSectionFilter || this.bookingServiceFilter || this.bookingResourceFilter);
    },
  ...
  methods: {
    async switchAppointmentView(view) {
      if (!['table', 'timeline'].includes(view) || this.appointmentView === view) return;

      if (view === 'timeline') {
        this.appointmentView = 'timeline';
        await this.refreshBookingTimelineSchedule();
        return;
      }

      const requestId = (this._appointmentViewSwitchRequest || 0) + 1;
      this._appointmentViewSwitchRequest = requestId;
      ...
      // Timeline and table share the same rows, but their cached representation
      // can be stale after edits or realtime events. Returning to the table must
      // always reconcile the current month with the server, just like a refresh.
      delete this.monthAppointmentsCache[month];
      delete this.monthDaysCache[month];
      await this.fetchData(true);
      if (requestId !== this._appointmentViewSwitchRequest) return;

      this.days.forEach(day => {
        const dayNumber = Number(day.dayNum);
        if (collapsedByDay.has(dayNumber)) day.collapsed = collapsedByDay.get(dayNumber);
      });
      this.syncAllDaysCollapsedState();
    },

    bookingSectionsForFilters(rootSectionId = '', subsectionId = '') {
      return this.enabledBookingSectionOptions.filter(section => {
        if (subsectionId && String(section.id) !== String(subsectionId)) return false;
        if (!rootSectionId) return true;
        return String(this.rootSectionIdFor(section.id) || section.id) === String(rootSectionId);
      });
    },

    bookingResourcesForFilters(rootSectionId = '', subsectionId = '') {
      const sections = this.bookingSectionsForFilters(rootSectionId, subsectionId);
      const resources = sections.flatMap(section => section.services)
        .flatMap(service => service.booking_resources || service.bookingResources || [])
        .filter(resource => resource.active !== false);
      const sectionIds = [...new Set(sections.flatMap(section => this.serviceSectionScopeIds([section.id])))];
      const sectionDoctors = (this.doctors || [])
        .filter(doctor => {
          ...
          return doctorSectionIds.some(sectionId => sectionIds.includes(sectionId));
        })
      ...
      const options = [...resources, ...sectionDoctors].map(resource => {
        ...
        const person = role === 'doctor'
          ? this.doctors.find(item => Number(item.id) === Number(resource.doctor_id))
          : this.staff.find(item => Number(item.id) === Number(resource.staff_id));
    ...
    onDraftBookingRootSectionChanged() {
      const subsectionExists = this.bookingSubsectionFilterOptions.some(section => String(section.id) === String(this.draftBookingServiceFilter));
      if (!subsectionExists) this.draftBookingServiceFilter = '';
      const resourceExists = this.bookingResourceFilterOptions.some(resource => resource.value === this.draftBookingResourceFilter);
      if (!resourceExists) this.draftBookingResourceFilter = '';
    },

    onDraftBookingServiceFilterChanged() {
      if (this.draftBookingServiceFilter) {
        const section = this.enabledBookingSectionOptions.find(item => String(item.id) === String(this.draftBookingServiceFilter));
        this.draftBookingRootSectionFilter = String(this.rootSectionIdFor(section?.id) || section?.id || this.draftBookingRootSectionFilter);
      }
      const resourceExists = this.bookingResourceFilterOptions.some(resource => resource.value === this.draftBookingResourceFilter);
      if (!resourceExists) this.draftBookingResourceFilter = '';
    },

    applyBookingTimelineFilters() {
      this.bookingRootSectionFilter = this.draftBookingRootSectionFilter;
      this.bookingServiceFilter = this.draftBookingServiceFilter;
      this.bookingResourceFilter = this.draftBookingResourceFilter;
      if (this.appointmentView === 'timeline') this.refreshBookingTimelineSchedule();
    },

    clearBookingTimelineFilters() {
      this.draftBookingRootSectionFilter = '';
      this.draftBookingServiceFilter = '';
      this.draftBookingResourceFilter = '';
      this.bookingRootSectionFilter = '';
      this.bookingServiceFilter = '';
    ...
    timelineTimeLabel(row) {
      ...
      if (!start) return "بدون ساعت";
      const span = this.timelineSlotSpan(row);
      if (span <= 1 || this.isEmptyAppointmentRow(row)) return start;
      const duration = this.bookingDurationMinutesForRow(row);
      return `${start} تا ${this.timeFromMinutes(this.minutesFromTime(start) + duration)}`;
    },
    ...
    timelineSlotSpan(row) {
      if (!row || this.isEmptyAppointmentRow(row) || !this.bookingServiceFilter) return 1;
      const baseInterval = Math.max(1, Number(this.clinicSchedule.interval_minutes || 15));
      const duration = Math.max(baseInterval, Number(this.bookingDurationMinutesForRow(row) || baseInterval));
      return Math.max(1, Math.min(24, Math.ceil(duration / baseInterval)));
    },
    ...
    applyTimelineHeaderDefaults(draft) {
      ...
      const selectedRootSectionId = String(this.bookingRootSectionFilter || '').trim();
      if (subsectionId) {
      ...
      } else if (selectedRootSectionId) {
        draft.serviceTypes = [selectedRootSectionId];
      }
    ...
    async saveTimelineModal() {
      ...
      // In the timeline form the consultant is selected once for the whole
      // appointment. Keep an explicitly selected per-service consultant, but
    ...
    async onAppointmentStatusSelected(row, event) {
      ...
      if (['کنسل شد', 'پاسخ نداد', 'پیگیری'].includes(status)) {
        const followupChoice = await this.promptAppointmentFollowup(row, status);
        if (followupChoice === 'cancelled') { event.target.value = row.status || ''; return; }
      }
      row.status = status;
    ...
    async promptAppointmentFollowup(row, reason) {
      if (!row?.appointmentId) {
        if (!await this.persistDirectAppointment(row)) return 'cancelled';
      }
      if (!row?.appointmentId) return 'cancelled';
      ...
      const result = await Swal.fire({
        ...
        showDenyButton: true,
        showCancelButton: true,
        confirmButtonText: 'ثبت در پیگیری‌ها',
        denyButtonText: 'نمی‌خواهم پیگیری کنم',
        cancelButtonText: 'انصراف از تغییر'
      });
      if (result.isDenied) return 'without-followup';
      if (!result.isConfirmed || !result.value) return 'cancelled';
      try {
    ...
    async onDoneChanged(row) {
      const done = String(row.done || '').trim();
      if (done === 'مشاوره') {
        const followupChoice = await this.promptAppointmentFollowup(row, 'مشاوره');
        if (followupChoice === 'cancelled') { row.done = ''; return; }
      }
    ...
    calculateTrackingFinancialRow(row) {
      ...
      this.expandedServices(row).forEach(service => {
        ...
        const consultantName = String(service.consultant || row.consultant || row.timelineConsultant || '').trim();
        const consultant = this.staff.find(s => String(s.name || '').trim() === consultantName);
    ...
    async fetchData(force = false) {
      ...
      try {
        ...
        if (force || !data) {
          const [appointmentsRes, hiddenDaysRes] = await Promise.all([
            axios.get("/api/appointments", { params: { month: targetMonth } }),
            axios.get("/api/appointments/hidden-days", { params: { month: targetMonth } })
          ]);
          data = Array.isArray(appointmentsRes.data) ? appointmentsRes.data : [];
          hiddenDaysData = Array.isArray(hiddenDaysRes.data) ? hiddenDaysRes.data : [];
          this.monthAppointmentsCache[targetMonth] = data;
        } else {
          const hiddenDaysRes = await axios.get("/api/appointments/hidden-days", {
        ...
        this.hiddenScheduleDayNumbers = hiddenDaysData.map(Number);
        const hiddenDays = new Set(this.hiddenScheduleDayNumbers);
        data = data.filter(item => !hiddenDays.has(Number(item.day_num)));
    ...
    calculateClinicProfitRow(row) {
      ...
      this.expandedServices(row).forEach(service => {
        ...
        const consultantName = String(service.consultant || row.consultant || row.timelineConsultant || '').trim();
        const consultant = this.staff.find(
          s => String(s.name || '').trim() === consultantName
        );
...
this.calculateFinalAmount(row)
    ...
    financialLinesForRow(row) {
      return (row?.services || []).flatMap((service, index) => {
        if (!String(service?.name || '').trim()) return [];
        const baseAmount = this.serviceLinePrice(service);
        const base = { key: `${service.name}-${index}`, service: String(service.name).trim(), section: '', subsection: '', parent_service: '', is_addon: false, quantity: Math.max(1, Number(service.cc || 1)), grossAmount: baseAmount, discountAmount: this.moneyToNumber(service.discount), netAmount: this.serviceNetAmount(service, baseAmount) };
        return [base, ...(service.addons || []).filter(addon => String(addon?.name || '').trim()).map((addon, addonIndex) => ({ ...base, key: `${base.key}-addon-${addon.addon_definition_id || addonIndex}`, service: String(addon.name).trim(), parent_service: base.service, is_addon: true, grossAmount: this.serviceLinePrice(addon), discountAmount: this.moneyToNumber(addon.discount), netAmount: this.serviceNetAmount(addon, this.serviceLinePrice(addon)) }))];
      });
    },

    async loadFinancialContext() {
      if (!this.activeFinancialRow?.appointmentId) return;
      try {
      ...
      } catch { this.financialPreviousDebts = []; this.financialTransactions = []; }
    },
    ...
    async saveFinancialPanel() {
      ...
      try {
        ...
        if (data.appointment?.lock_version) row.lockVersion = Number(data.appointment.lock_version);
        if (data.appointment?.payment_details) row.paymentDetails = this.normalizePaymentDetails(data.appointment.payment_details);
        row.walletBalance = Number(data.wallet_balance || row.walletBalance || 0);
        this.financialDepositLines = this.depositLinesForRow(row);
        this.financialInvoiceLines = this.financialLinesForRow(row);
        await this.loadFinancialContext();
        await this.loadFinancialDepositHistory();
    ...
    bookingScheduleAllowsRow(day, row) {
      ...
      let resources = sections.flatMap(section => section.services)
        .flatMap(service => service.booking_resources || service.bookingResources || []);
      if (this.bookingResourceFilter) {
        const [type, id] = String(this.bookingResourceFilter).split(':');
        resources = resources.filter(resource => type === 'doctor'
    ...
    activeBookingServicesForDuration() {
      if (!this.bookingServiceFilter) return [];
    ...
    getFilteredRows(day) {
      let rows = day.rows.filter(row => {
        ...
        const bookingServiceOk = !this.bookingServiceFilter || keepEmptyBookingSlot || services.some(service => {
          const sectionId = service.sectionId || service.section_id || this.sectionIdForService(service.name, row);
        ...
        return statusOk && sourceOk && doneOk && genderOk && appointmentSmsOk && infoSmsOk && sectionOk && doctorOk && consultantOk && subsectionOk && amountOk && cardOk && debtorOk && emptyTimeOk && bookingRootSectionOk && bookingServiceOk && bookingResourceOk && timelineSlotOk && bookingScheduleOk && bookingCapacityOk;
      });
...
.booking-filter-apply { border:1px solid #2563eb; background:#2563eb; color:#fff; box-shadow:0 3px 8px rgba(37,99,235,.2); }
.booking-filter-clear { border:1px solid #cbd5e1; background:#fff; color:#64748b; }
.booking-filter-apply:disabled,.booking-filter-clear:disabled { opacity:.58; cursor:wait; }
...
.appointment-timeline .timeline-add-card {
  flex: 0 0 var(--timeline-card-width, 126px) !important;
  width: var(--timeline-card-width, 126px) !important;
  height: 142px;
  border-radius: 10px;
}
...
.main-schedule-table tr.search-highlight-row input,
.main-schedule-table tr.search-highlight-row select,
.main-schedule-table tr.search-highlight-row textarea {
...
.financial-invoice-card,.financial-accordion-section{margin-top:12px;border:1px solid #e2e8f0;border-radius:14px;background:#fff;overflow:hidden}.financial-accordion-head{width:100%;display:flex;align-items:center;justify-content:space-between;padding:13px 14px;border:0;background:#f8fafc;color:#172554;font-family:inherit;font-size:12px;font-weight:1000;cursor:pointer}.financial-accordion-head b{color:#2563eb;font-size:11px}.financial-invoice-table-wrap{padding:10px}.financial-invoice-table-wrap table{width:100%;border-collapse:collapse;font-size:10px}.financial-invoice-table-wrap th,.financial-invoice-table-wrap td{padding:9px 7px;border-bottom:1px solid #eef2f7;text-align:right}.financial-invoice-table-wrap th{color:#64748b;background:#f8fafc;font-size:9px}.financial-invoice-table-wrap td small{display:block;color:#94a3b8;font-size:8px;margin-top:3px}.financial-payable-total{display:flex;justify-content:space-between;padding:12px 6px 3px;color:#475569;font-size:11px;font-weight:900}.financial-payable-total strong{color:#2563eb;font-size:15px}.financial-previous-debts{display:grid;gap:7px;margin-top:11px}.financial-previous-debts article{display:grid;grid-template-columns:1fr auto auto;align-items:center;gap:8px;padding:9px;border:1px solid #fecaca;border-radius:10px;background:#fff7f7;color:#7f1d1d;font-size:10px}.financial-previous-debts button{height:29px;padding:0 9px;border:0;border-radius:8px;background:#16a34a;color:#fff;font-family:inherit;font-size:9px;font-weight:900;cursor:pointer}.payment-open>.financial-accordion-head{background:#eff6ff;color:#1d4ed8}
.financial-method-choices{display:flex;flex-wrap:wrap;gap:7px;padding:12px 12px 2px}.financial-method-choices button{height:34px;display:flex;align-items:center;gap:6px;padding:0 11px;border:1px solid #cbd5e1;border-radius:10px;background:#fff;color:#475569;font-family:inherit;font-size:10px;font-weight:900;cursor:pointer}.financial-method-choices button span{width:17px;height:17px;display:grid;place-items:center;border-radius:5px;background:#e2e8f0}.financial-method-choices button.active{border-color:#60a5fa;background:#eff6ff;color:#1d4ed8}.financial-method-choices button.active span{background:#2563eb;color:#fff}
.financial-panel-overlay{z-index:2147483600!important;padding:clamp(76px,9vh,110px) 16px 18px!important;place-items:start center!important;overflow:auto}.financial-panel{width:min(760px,96vw)!important;max-height:calc(100vh - 94px)!important;padding:0!important;border-radius:24px!important;overflow:auto!important}.financial-panel>header{position:sticky;top:0;z-index:5;margin:0!important;padding:19px 23px 16px!important;background:linear-gradient(135deg,#eff6ff,#fff 55%,#ecfdf5);border-bottom:1px solid #e2e8f0}.financial-panel>.financial-summary,.financial-panel>.financial-invoice-card,.financial-panel>.financial-accordion-section,.financial-panel>.financial-wallet-settle,.financial-panel>.financial-debt-payment,.financial-panel>footer,.financial-panel>.financial-patient-warning{margin-left:22px;margin-right:22px}.financial-panel>.financial-summary{margin-top:18px}.financial-panel>footer{padding-bottom:20px}

/* Financial workspace: spacious, predictable cards instead of compressed inline controls. */
.financial-panel{width:min(860px,96vw)!important;background:#f8fafc!important;color:#0f172a}.financial-panel>header{align-items:center!important;padding:22px 28px!important;background:linear-gradient(135deg,#eff6ff 0%,#fff 58%,#ecfdf5 100%)!important}.financial-panel>header h3{margin:6px 0 0!important;font-size:21px}.financial-panel>header button{display:grid;place-items:center;cursor:pointer;transition:.18s ease}.financial-panel>header button:hover{background:#e2e8f0;color:#0f172a;transform:rotate(4deg)}
