...
abstract class AppointmentMoneyCompareFilter implements DynamicReportQueryFilter
{
    ...
    public function apply(Builder $query, array $filters): void
    {
        ...
        $query->whereExists(function ($appointments) use ($number, $op) {
            $appointments->selectRaw('1')->from('appointments as money_'.$this->key())
                ->whereRaw($this->expression('money_'.$this->key()).' '.$op.' ?', [$number])
                ->where(function ($link) {
